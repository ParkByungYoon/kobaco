...
def run(args):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"[Epoch {epoch}/{num_epochs}] train_loss: {round(train_loss, 5)}, test_loss: {round(test_loss, 5)}, test_recall@{topk}: {round(recall, 5)}, test_precision@{topk}: {round(precision, 5)}, test_ndcg@{topk}: {round(ndcg, 5)}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Argparse LLM4Rec')
    parser.add_argument('--data_path', default='./data/ml-latest-small')
